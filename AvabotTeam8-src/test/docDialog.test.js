...
describe("DocDialog", () => {
  ...
  it("tests no", async () => {
    ...
    assert.strictEqual(
      reply.text,
      "How can I help with the document? (1) summarize it, (2) extract table, or (3) QnA about it"
    );
  });
  ...
  it("tests answer", async () => {
    ...
    assert.strictEqual(
      reply.text,
      "Anything else for the document?\n\n   1. summarize it\n   2. extract table\n   3. QnA about it\n   4. no"
    );
  });
  ...
  it("tests Begin", async () => {
    ...
    await w.beginStep({
      options: { sum: "sum", query: "query", form: "form", filepath: "path" },
      prompt: function () {
        return;
      },
    });
