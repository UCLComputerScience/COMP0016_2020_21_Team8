...
describe("MainDialog", () => {
  ...
  it("tests docMissing", async () => {
    ...
    assert.strictEqual(
      reply.text,
      "What else can I do for you? (1) ask a question, (2) process a document, or (3) recognize an image"
    );
  });
  ...
  it("tests downloadAttach", async () => {
    const sut = new MainDialog();
    let a = await sut.downloadAttachmentAndWrite({
      contentType: "a/b",
      contentUrl: "url",
      name: "hi",
    });
  ...
  it("tests docStepPdf", async () => {
    ...
    let a = await sut
      .docStep({
        result: [{ contentUrl: "url", contentType: "application/pdf" }],
        activity: "",
        next: function () {
          return;
        },
  ...
  it("tests docStepImage", async () => {
    ...
    let a = await sut
      .docStep({
        result: [{ contentUrl: "url", contentType: "application/png" }],
        activity: "",
        context: "",
        next: function () {
          return;
  ...
  it("tests handleAttachment", async () => {
    const path = require("path");
    const sut = new MainDialog();
    let a = await sut.handleIncomingAttachment({
      activity: {
        attachments: [
          {
  ...
  it("tests promptValidatorPdf", async () => {
    ...
    let a = await sut.promptValidator({
      ...
      options: { prompt: "Please attach a document in pdf." },
    });
  ...
  it("tests promptValidatorImage", async () => {
    const sut = new MainDialog();
    let a = await sut.promptValidator({
      recognized: { succeeded: true, value: [{ contentType: "image/png" }] },
  ...
  it("tests errorAttachment", async () => {
    ...
    let a = await sut.handleIncomingAttachment({
      activity: {
        attachments: [
          { contentType: "application/nvm", contentUrl: "nvm", name: "nvm" },
        ],
      },
      sendActivity: function () {
        return;
  ...
  it("tests docStepPicReq", async () => {
    const sut = new MainDialog();
    let a = await sut.docStep({
      result: [{ contentUrl: "nvm", contentType: "image/png" }],
      context: {
        activity: {
          attachments: [
            {
  ...
  it("tests docStepSendReq", async () => {
    const sut = new MainDialog();
    let a = await sut.docStep({
      result: [{ contentUrl: "nvm", contentType: "image/jpeg" }],
      context: {
        activity: {
          attachments: [
            {
